...
class UserApi(Resource):  # 声明User资源
    ...
    def delete(self):
        ...
        # user=queryById(User,id)  #是否考虑id不存在的情况
        # delete(user)
...
class MusicApi(Resource):
    parser = reqparse.RequestParser()
    parser.add_argument('key',dest='name',type=str,required=True,help='必须提供name')
    parser.add_argument('id',type=int,help='请确定id的参数是数值型')
    parser.add_argument('tag',action='append',required=True,help='至少提供一个tag标签')
    parser.add_argument('session',location='cookies',required=True,help='至少提供一个cookie')
#定制输出
    music_fieids={
        'id':fields.Integer,
        'name':fields.String,
    ...
    out_fields={
        ...
        'data':fields.Nested(music_fieids)
